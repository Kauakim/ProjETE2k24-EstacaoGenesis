...
#ifdef DEBUG
	 PB->S << Offset << "(ClearScheduledMessage = "<< ClearScheduledMessage << ")" << endl;
#endif
	  ...
	  for (unsigned int i = 0; i < PCore->Subscriptions.size (); i++)
		{
		  ...
		  if (PCore->GetPeerAppTupleIndex (PS->Publisher.Values[2], Index) != OK)
			{
			  PB->S << Offset1 << "(Warning: The publisher is unknown. Checking for unkown EPGS case. If the peer is an IoTTestApp, disregard this message)" << endl;
			  ...
			  if (StoreFlag == true && PS->Publisher.Values.at (3) == "NULL")
				{
				  ...
				  PB->S << Offset1 << "(Registered the EPGS application as a peer with index "<<Index<<", since a contract was received.)" << endl;
				}
			  else
				{
				  PB->S << Offset1 << "(The subscription is from an IoTTestApp not registered. Going to delete the subscription and disregard the message content.)" << endl;
  ...
  if (Temp == "Service_Offer")
	{
	  ...
	  // Modified in Sep 2024 to provide multiple aservice_acceptance messages for EPGS

	  //Send multiple acceptance messages when the offer comes from EPGS
	  if (_Publisher_LN == "EPGS")
		{
		  ...
		  for (unsigned int h = 0; h < PCore->PeerTuples.size (); h++)
			{
			  ...
			  if (PCore->PeerTuples[h]->LN == "IoTTestApp")
				{
				  // Set the service acceptance payload
		 ...
		 // Schedule submission of 2 acceptance messages for EPGS
		  for (unsigned int v = 0; v < 2; v++)
			{
				PublishAcceptance = NULL;
				...
				double X=GetTime () + v*5;

				PB->S << Offset1 << "Time = "<< X << endl;

				// Creating a new message
				PB->PP->NewMessage (X, 1, false, PublishAcceptance);
	...
	// Send only one acceptance when the message comes from IoTTestApp
