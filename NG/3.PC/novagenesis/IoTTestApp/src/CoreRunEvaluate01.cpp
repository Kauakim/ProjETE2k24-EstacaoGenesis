...
int CoreRunEvaluate01::ProcessJsonFile (string _Publisher_LN, string _FileName, string _HashPayload)
{
  ...
  PCore = (Core *)PB;
	...
	// Ler a linha inteira do arquivo
	std::string line;
	std::getline(F1, line);

	// Função lambda para extrair o valor dado uma chave
	auto extractValue = [&](const std::string& key) -> int {
		size_t pos = line.find("\"" + key + "\":");
		if (pos != std::string::npos) {
			pos += key.length() + 3; // Avançar após a chave e '":'
			// Pular espaços em branco
			while (pos < line.size() && isspace(line[pos])) pos++;
			// Ler o valor até a próxima vírgula ou chave fechando
			size_t pos_end = line.find_first_of(",}", pos);
			std::string value_str = line.substr(pos, pos_end - pos);
	...
	double temperatura = extractValue("Temperatura")/10;
	double umidade = extractValue("Umidade")/10;
	double pressao = extractValue("Pressao")/10;
	double radiacao_solar = extractValue("Rad_solar");
	double uv = extractValue("Uv");
	double velocidade_vento = extractValue("Vel_vento")/10000;
	double inclinacao_vento = extractValue("Inc_vento");
	double pluv = extractValue("Pluv")*0.2;
	int porta = extractValue("Porta");
	double ponto_de_orvalho = 0;

	F2.OpenOutputFile ();

	// Escrever os valores extraídos no arquivo de saída
	F2 << std::setprecision(10) << GetTime() << " " << _Publisher_LN << " "
	<< temperatura << " " << umidade << " " << pressao << " " << radiacao_solar << " "
	<< uv << " " << velocidade_vento << " " << inclinacao_vento << " "
	<< pluv << " " << porta << " " << ponto_de_orvalho << std::endl;

	F2.CloseFile ();
	F1.CloseFile ();

	// Processa cada parâmetro se a condição do filtro for atendida
	if(temperatura > -20 && temperatura < 50){
		PCore->ptemperatura->Sample (temperatura);
		PCore->ptemperatura->CalculateArithmetic ();
		PCore->ptemperatura->SampleToFile (GetTime());
		PB->S << Offset1 << "(Temperatura média = " << PCore->ptemperatura->GetMean() << ")" << endl;
	}

	if(umidade > 0 && umidade <= 100){
		PCore->pumidade->Sample (umidade);
		PCore->pumidade->CalculateArithmetic ();
		PCore->pumidade->SampleToFile (GetTime());
		PB->S << Offset1 << "(Umidade média = " << PCore->pumidade->GetMean() << ")" << endl;
	}
  	
	if(pressao >= 700 & pressao <= 1000){
		PCore->ppressao->Sample (pressao);
		PCore->ppressao->CalculateArithmetic ();
		PCore->ppressao->SampleToFile (GetTime());
		PB->S << Offset1 << "(Pressão média = " << PCore->ppressao->GetMean() << ")" << endl;
	}
	
	if(radiacao_solar <= 5000){
		PCore->pradiacao_solar->Sample (radiacao_solar);
		PCore->pradiacao_solar->CalculateArithmetic ();
		PCore->pradiacao_solar->SampleToFile (GetTime());
		PB->S << Offset1 << "(Radiação média = " << PCore->pradiacao_solar->GetMean() << ")" << endl;
	}
	
	if(uv <= 4000){
		PCore->puv->Sample (uv);
		PCore->puv->CalculateArithmetic ();
		PCore->puv->SampleToFile (GetTime());
		PB->S << Offset1 << "(UV médio = " << PCore->puv->GetMean() << ")" << endl;
	}
	
	if(velocidade_vento < 50){
		PCore->pvelocidade_vento->Sample (velocidade_vento);
		PCore->pvelocidade_vento->CalculateArithmetic ();
		PCore->pvelocidade_vento->SampleToFile (GetTime());
		PB->S << Offset1 << "(Velocidade do vento médio = " << PCore->pvelocidade_vento->GetMean() << ")" << endl;
		PB->S << Offset1 << "(Rajada do vento = " << PCore->pvelocidade_vento->GetMax() << ")" << endl;
	}
	
	if(inclinacao_vento >= 0 && inclinacao_vento <= 360){
		PCore->pinclinacao_vento->Sample (inclinacao_vento);
		PCore->pinclinacao_vento->CalculateArithmetic ();
		PCore->pinclinacao_vento->SampleToFile (GetTime());
		PB->S << Offset1 << "(Inclinação de vento médio = " << PCore->pinclinacao_vento->GetMean() << ")" << endl;
	}
	
	if(pluv >= 0){
		PCore->ppluv_count->Sample (pluv);
		PCore->ppluv_count->CalculateArithmetic ();
		PCore->ppluv_count->SampleToFile (GetTime());
		PB->S << Offset1 << "(Contagem do pluviometro total = " << PCore->ppluv_count->GetSum() << ")" << endl;
	}

	if(porta == 1 || porta == 0){
		PCore->pbutton_state->Sample (porta);  	
		PCore->pbutton_state->CalculateWeighted (GetTime());
		PCore->pbutton_state->SampleToFile (GetTime());
		PB->S << Offset1 << "(Estado do botao médio = " << PCore->pbutton_state->GetMean() << ")" << endl;
	}
	

	// Cálculo para o ponto de orvalho
	double alpha = 0;
	const double a = 17.27;
	const double b = 237.7;

	if(umidade > 0 && umidade <= 100 && temperatura != -b){
		alpha = (a * temperatura) / (b + temperatura) + log(umidade / 100.0);
	}

	if(alpha != a){
		ponto_de_orvalho = (b * alpha) / (a - alpha);

		PCore->porv->Sample (ponto_de_orvalho);
		PCore->porv->CalculateArithmetic ();
		PCore->porv->SampleToFile(GetTime());
		PB->S << Offset1 << "(Ponto de orvalho médio = " << PCore->porv->GetMean() << ")" << endl;
	}
