...
function ResetSimulacao() {
  window.location.href = "../gemeo.html";  
}
...
let DadosData = "12/12/2019";
let DadosHora = "12:00:00";
let DadosEstilo = "yellow";
let CorGraficos = 'var(--dash-p1)';
let QualidadeGraficos = "Alto";
let DadosHoraNumero = parseInt(DadosHora.substring(0, 2));

// Função que busca no localStorage ou usa os valores padrão
function inicializarVariaveis() {
  DadosEstacao = localStorage.getItem("DadosEstacao") ?? DadosEstacao;
  DadosData = localStorage.getItem("DadosData") ?? DadosData;
  DadosHora = localStorage.getItem("DadosHora") ?? DadosHora;
  DadosEstilo = localStorage.getItem("DadosEstilo") ?? DadosEstilo;
  DadosHoraNumero = parseInt(DadosHora.substring(0, 2));
  QualidadeGraficos = localStorage.getItem("QualidadeGraficos") ?? QualidadeGraficos;
  CorGraficos = localStorage.getItem("CorGraficos") ?? CorGraficos;
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  closeConfig.addEventListener('click', () => {
      ...
      DadosEstacao = document.getElementById('estacao').value;
      DadosData = document.getElementById('data').value;
      DadosHora = document.getElementById('hora').value;
      DadosData = dayjs(DadosData).format('MM/DD/YYYY');
      DadosHora = DadosHora + ":00";
      DadosHoraNumero = parseInt(DadosHora.substring(0, 2));
      ...
      QualidadeGraficos = document.getElementById('qualidade').value;

      // Coleta a cor selecionada (estilo)
      let DadosEstilo = 'yellow';
      estiloElementos.forEach((elemento) => {
          if (elemento.classList.contains('selected')) {
              DadosEstilo = elemento.getAttribute('data-color') || 'Cor não definida'; // Captura a cor do atributo data-color
          }
      });
      ...
      localStorage.setItem("DadosEstacao", DadosEstacao);
      localStorage.setItem("DadosData", DadosData);
      localStorage.setItem("DadosHora", DadosHora);
      localStorage.setItem("DadosEstilo", DadosEstilo);
      localStorage.setItem("QualidadeGraficos", QualidadeGraficos);
      localStorage.setItem("CorGraficos", CorGraficos);
...
async function BuscarHoraNoHistorico() {
  try {
      ...
      const response = await fetch(`http://localhost:55000/historico?dia=${DadosData}&estacao=${DadosEstacao}`);
...
async function BuscarHoraNaPrevisao() {
  try {
      ...
      try {
          ...
          console.log(NumeroLinhasDesejadasPrevisao);
...
function Simulacao(){
  ...
  // Gráficos
  console.log(`Cor dos Gráficos: ${CorGraficos}`);
  console.log(`Paleta: ${paleta.href}`);

  MiniGraficoCircularSVG('#GraficoChuva', Chuva, 30, CorGraficos, 'Chuva', IconeChuva, ' mm');
  MiniGraficoCircularSVG('#GraficoRadiacaoSolar', RadiacaoSolar, 4000, CorGraficos, 'Radiacao Solar', IconeRadiacaoSolar, ' Kj');
  MiniGraficoCircularSVG('#GraficoTemperaturaMedia', TemperaturaMedia, 40, CorGraficos, 'Temperatura Media', IconeTemperaturaMedia, ' °C');
  MiniGraficoCircularSVG('#GraficoPressaoMedia', PressaoMedia, 1100, CorGraficos, 'Pressao Atmosferica Media', IconePressaoAtomsfericaMedia, ' mB');
  ...
  MiniGraficoCircularSVG('#GraficoVelocidadeVento', Vento, 10, CorGraficos, 'Velocidade do Vento', IconeVento, ' m/s');
  MiniGraficoCircularSVG('#GraficoVelocidadeRajadaVento', VentoRajada, 10, CorGraficos, 'Velocidade de Rajada do Vento', IconeVentoRajada, ' m/s');
